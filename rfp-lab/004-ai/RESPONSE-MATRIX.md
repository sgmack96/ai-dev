...
| GTW-05 | Content Safety / PII Controls | **NC** | AI Gateway DLP | ❌ | Confirmed via live API check: `dlp.enabled: true` on `se-intel-gateway`, but `dlp.policies: []` — the feature is toggled on with zero policies attached, so it currently does nothing. **Attempted to close this gap on 2026-07-08; blocked at two levels, not abandoned at either:** (1) *Access blocker:* the API token available for this lab is scoped to Workers AI only (`Account > Workers AI > Edit`) and returned an authentication error against the DLP profiles endpoint — closing this specific blocker needs a broader-scoped token or dashboard access. (2) *Architecture blocker, found by reading [Cloudflare's DLP docs](https://developers.cloudflare.com/ai-gateway/features/dlp/) directly:* even with the right token, DLP policies are configured **per-gateway, not per-request** — there is no header or parameter to select a policy per tenant. `se-intel` routes every org through one shared gateway (`se-intel-gateway`), so turning on DLP would apply one policy uniformly across all tenants; true per-tenant DLP variance would require standing up separate gateways per org, the same shared-vs-per-tenant fork already navigated once for Vectorize in Week 1 — except this time the platform offers no metadata-filter middle ground. Separately, the docs confirm DLP **response scanning buffers the entire streamed response before releasing any of it to the client**, which would directly undo the sub-100ms-TTFB streaming architecture documented in STUDY.md Chapter 7 if enabled on the same route. This is not a gap that closes with more API permissions alone — it's a real architecture decision (per-org gateways, or `Check: Request`-only to avoid the streaming conflict) that hasn't been made yet. |
...
- Gateway-level rate limiting (GTW-04) and content safety/DLP (GTW-05) — GTW-05 was actively attempted this session and found to be blocked at two levels: an access-control blocker (API token scope) and a real architecture decision (per-gateway DLP policies conflict with se-intel's single shared gateway across tenants, and DLP response-scanning conflicts with the streaming architecture) — see GTW-05's full response for the sourcing.
